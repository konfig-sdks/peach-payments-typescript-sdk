...
import { readableStreamToString, PeachPaymentsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PeachPaymentsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PeachPaymentsError(e, e.response?.data)
                }
                if (innerError instanceof PeachPaymentsError) {
                    // Got "PeachPaymentsError" from the above try block
                    throw innerError;
                }
